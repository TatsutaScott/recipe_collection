...
function findWhole(frac){
    return frac.numerator > frac.denominator || frac.numerator === frac.denominator? Math.floor(frac.numerator / frac.denominator) : 0;
}

function findNumerator(frac){
    const thing = frac.numerator > frac.denominator ? frac.numerator % frac.denominator : frac.numerator;
    console.log(thing);
    return thing;
}
